...
# run the app
# without seed: npm run p-mg-prod && npm run p-gen && node dist/app.js
# with seed: npm run p-mg-prod && npm run p-gen && npm run seed && node dist/app.js
# npm run rollback && [if failed we will rollback first]
CMD ["/bin/sh", "-c", "sleep 20 && echo 'sleep end, start deploy.....' && npm run p-mg-prod && npm run p-gen && node dist/app.js"]
...
# with seed: npm run p-mg-prod && npm run p-gen && npm run seed && node dist/app.js
# npm run rollback && [if failed we will rollback first]
CMD ["/bin/sh", "-c", "sleep 5 && echo 'sleep end, start deploy.....' && npm run p-mg-prod && npm run p-gen && node dist/app.js"]